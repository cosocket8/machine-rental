  ...
  return (
    <div className="min-h-screen bg-gray-100">
      <div className="bg-white shadow px-6 py-4 flex justify-between items-center">
        <h1 className="text-xl font-bold text-blue-600">MachineRent</h1>
        <div className="flex items-center gap-3">
          <button onClick={() => navigate('/browse')}
            className="bg-green-600 text-white px-4 py-2 rounded-lg text-sm">🔍 Rent</button>
          {(profile?.role === 'owner' || profile?.role === 'admin') && (
            <button onClick={() => navigate('/list-machine')}
              className="bg-blue-600 text-white px-4 py-2 rounded-lg text-sm">+ List</button>
          )}
          <span className="text-sm text-gray-600">{profile?.full_name} ({profile?.role})</span>
